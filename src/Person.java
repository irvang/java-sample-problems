public class Person {
    ...
    public Person (String name, Date dob) throws Exception {
        if(name == null) {
    ...
    public Date getBirthdayIn (int year) throws Exception {
        Date birthdayIn = new Date(this.dob.getMonth(), this.dob.getDay(), year);

        return birthdayIn;
    }

    public int getAgeOn(Date date) throws Exception {
        if(date.isBefore(this.dob)){
            throw new Exception("The date cannot be a time before the person's DOB");
        }

        Date birthdayOnYear = new Date(this.dob.getMonth(), this.dob.getDay(), date.getYear());

        // 10/5/2013
        // if date is before than birthdayOnYear, date.getYear() - this.dob .getYear() - 1
