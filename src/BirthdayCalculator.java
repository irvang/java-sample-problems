...
public class BirthdayCalculator {
	/*
	 * getPeople - reads in the information (name and dob) for count different
	 * people using the specified Scanner and returns an array of Person objects
	...
	/*
	 * printBirthdays - takes an array of Person objects and a year as
	 * parameters, and prints the names of the people in the array, along with
	 * their birthdays and ages in the specified year.
	 * 
	 * See the sample runs for what this method's output should look like.
	 */
	public static void printBirthdays(Person[] people, int year) {
		/*
		 * Replace the line below with your implementation of this method. See
		 * part j of the assignment for more detail.
		 */
		// System.out.println("not yet implemented");

		for (int i = 0; i < people.length; i++) {
			Person currentPerson = people[i];
			Date birthdayInYear = currentPerson.getBirthdayIn(year);
			int ageInYear = currentPerson.getAgeOn(birthdayInYear);
			System.out.println(currentPerson.getName() + ": "
				+ birthdayInYear.toString() + " (" + ageInYear + " years old)");
		}
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*
			 * Create a Date object for today. You don't need to understand this
			 * code.
			 */
			GregorianCalendar cal = new GregorianCalendar();
			Date today = new Date(cal.get(cal.MONTH) + 1,
				cal.get(cal.DAY_OF_MONTH), cal.get(cal.YEAR));
